...
export default function CalendarView({
  userName, events, userAvailability, allAvailability, saving,
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-4"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "#707090", letterSpacing: "0.1em" }}>돈내고갇히는팸</p>
          </div>
        </div>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center py-6 px-4">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <div className="grid grid-cols-7 mb-2">
            {WEEKDAYS.map((day, i) => (
              <div key={day} className="text-center py-2 text-xs tracking-widest"
                style={{ fontFamily: "var(--font-cinzel)", color: i === 5 ? "#a07835" : i === 6 ? "#803030" : "#707090" }}>
                {day}
              </div>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            {mode === "availability" ? (
              <motion.div key="avail" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
                className="mt-6 space-y-3">
                <div className="flex items-center justify-center gap-6 text-xs" style={{ color: "#707090" }}>
                  <div className="flex items-center gap-1.5">
                    <div className="w-3 h-3 rounded-sm" style={{ background: "rgba(212,168,67,0.22)", border: "1px solid rgba(212,168,67,0.55)" }} />
                    내가 가능한 날
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px]" style={{ color: "rgba(212,168,67,0.65)" }}>+N</span>
                    다른 멤버 수
                  </div>
                </div>
                ...
                <button onClick={handleSave} disabled={saving}
                  ...
                  style={{
                    fontFamily: "var(--font-cinzel)",
                    background: savedFeedback ? "rgba(80,200,100,0.12)" : "rgba(212,168,67,0.12)",
                    border: savedFeedback ? "1px solid rgba(80,200,100,0.45)" : "1px solid rgba(212,168,67,0.45)",
                    color: savedFeedback ? "#60d080" : "#d4a843",
                  }}
                  onMouseEnter={(e) => { if (!savedFeedback) { e.currentTarget.style.background = "rgba(212,168,67,0.22)"; e.currentTarget.style.boxShadow = "0 0 20px rgba(212,168,67,0.2)"; } }}
                  onMouseLeave={(e) => { if (!savedFeedback) { e.currentTarget.style.background = "rgba(212,168,67,0.12)"; e.currentTarget.style.boxShadow = "none"; } }}>
                  {savedFeedback ? <><CheckCircle size={14} /> 저장 완료!</> : <><Save size={14} /> {saving ? "저장 중..." : "가용일 저장"}</>}
                </button>
              ...
              <motion.div key="ev" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
                className="mt-6 space-y-3">
                <div className="flex items-center justify-center gap-6 text-xs" style={{ color: "#707090" }}>
                  <div className="flex items-center gap-1.5">
                ...
                {totalEvents > 0 && (
                  <div className="rounded-lg overflow-hidden" style={{ border: "1px solid #383858" }}>
                    <div className="px-4 py-2.5" style={{ background: "#181830", borderBottom: "1px solid #383858" }}>
                      <p className="text-xs tracking-widest" style={{ color: "#a07835", fontFamily: "var(--font-cinzel)" }}>예정 일정</p>
                    </div>
                    <div className="divide-y" style={{ borderColor: "#1e1e38" }}>
                      {Object.entries(events)
                        .filter(([key]) => new Date(key) >= new Date(new Date().setHours(0, 0, 0, 0)))
                          ...
                          return (
                            <div key={dateKey}
                              ...
                              <div className="text-center px-2 py-0.5 rounded flex-shrink-0"
                                style={{ background: "rgba(212,168,67,0.12)", minWidth: 40 }}>
                                <p className="text-xs font-semibold" style={{ color: "#d4a843", fontFamily: "var(--font-cinzel)" }}>
                                  {String(d).padStart(2, "0")}
                                </p>
                                <p className="text-[9px]" style={{ color: "#a07835" }}>{MONTHS_KO[m - 1]}</p>
                              </div>
                              <div className="flex-1 min-w-0">
                                {dayEvents.slice(0, 2).map((ev) => (
                                  <div key={ev.id} className="flex items-center gap-2">
                                    <span className="w-1 h-1 rounded-full flex-shrink-0" style={{ background: "#d4a843" }} />
                                    <span className="text-xs truncate" style={{ color: "#eeeef8" }}>{ev.title}</span>
                                    {ev.time && <span className="text-[10px] flex-shrink-0" style={{ color: "#707090" }}>{ev.time}</span>}
                                  </div>
                                ))}
                              </div>
                            </div>
                ...
                <p className="text-xs text-center" style={{ color: "#707090" }}>날짜를 클릭해서 일정을 추가하세요</p>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
